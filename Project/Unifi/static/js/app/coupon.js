define(function(require,exports,module){
    ...
    module.exports= Backbone.View.extend({
        ...
        events: {
            "click .district span":"switchAreaCode",
        ...
        signincoupon:function(){
            if(this.checkMobile()) {
                var mobile = this.$el.find('#mobile').val();
                var token=this.$el.find("#token").val()||self.getQueryStringByName("token");
                var areacode=this.$el.find('.district .current').text();
                App.showLoading();
                signInCouponModel.set({"mobile": areacode+mobile})//组织参数
                signInCouponModel.exec().then(function (data) {
                    App.hideLoading();
                    if (data && data.code == 0) {
                        //成功跳转
                        App.jump("/unifi/couponsuccess");
                    } else {
                        App.showToast(data.msg);
                    }
                }).catch(function () {
                    App.hideLoading();
                    App.showToast("领券失败,请重试",2000);
                })
            }
        }
    })
